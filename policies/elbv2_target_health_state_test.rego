...
target_health_input_missing_state := {
	"schema_version": "v1",
	"source": "aws-elbv2",
	"account": {"account_id": "123456789012"},
	"region": {"name": "us-east-1"},
...
test_unknown_target_health_state_skipped if {
	count(policy.violation) == 0 with input as target_health_input("weird-future-state")
	policy.skip_reason with input as target_health_input("weird-future-state")
}

test_missing_target_health_state_skipped if {
	count(policy.violation) == 0 with input as target_health_input_missing_state
	policy.skip_reason with input as target_health_input_missing_state
}
